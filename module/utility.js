...
(function($) {
  ...
  colorscript.util = {
    ...
    turnOffLineNumber: function() {
      ...
      $('div.linenumber').hide();
      $('.editor').width(525);
      $('.code-text p').css('margin-left', 20);
    },
    ...
    changeLineNumber: function(editorId) {
      ...
      var ol = $(editor + 'div.linenumber ol');

      /* div가 한개 남았을때 백스페이스로 지울경우 div가 지워지면서 갯수가 0이 된다.
      이때 div를 하나 추가해주고 div_count를 강제로 1로 변환해준다.*/
      var p_count_in_pre = ($(editor + 'pre.code-text p').length == 0) ? (function() {
        $(editor + 'pre.code-text').find('br').remove().end().append("<p id='first_p'>\n</p>"); // br을 p로 바꿔준다
      ...
      })() : $(editor + 'pre.code-text p').length;

      var linenumber = $(editor + 'div.linenumber ol li').length;
